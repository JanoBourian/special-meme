        ...
        if has_minimum_info:
            try:
                ...
                for k, v in self.kwargs.items():
                    if not v.get("Authorization", ""):
                        logging.warning(f"{k.upper()}: {v}")
                response = requests.request(
                    method=self.method, url=self.url, **self.kwargs
                )
                logging.warning(f"RESPONSE_STATUS_CODE: {response.status_code}")
                logging.warning(f"RESPONSE_CONTENT: {response.content}")
            ...
            logging.warning(
                f"You don't have enough info for make request at {self.endpoint_name}. Please verify that the url or method are correct"
            )
            response = Response()
